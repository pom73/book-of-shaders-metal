...
//	float y  = abs(sin(2*PI*st.x+uniforms.time));
//	float y  = sin(st.x*uniforms.time);	
//	float y  = sin(3*PI*st.x);	
//	float y  = fract(sin(2*PI*st.x));	
//	float y  = ceil(sin(2*PI*st.x));	
	float y  = ceil(sin(2*PI*st.x)) + floor(sin(2*PI*st.x));	
...
//    float lineCoverage = line(st, 0.01f);
    float lineCoverage = plot(st, y, 0.05f);
